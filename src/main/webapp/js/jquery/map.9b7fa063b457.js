/* SC: map.js */
/* Required by _wid_map.html
   used in provider_map.html, provider_map_full.html,
           member_detail.html, map_county.html */
/* Include in:
  block js_voice_chat
    <script>
      var $person_id = {{ provider.id }};
      var $member_detail_url = "{% url 'member_detail' pk=1 %}"
      var $state = "{{ county_data.state }}"
      var $county = "{{ county_data.county }}"
    </script>
    <script src="{% static 'healthcare/js/rcg/map.js' %}"></script>
    <script src="https://maps.googleapis.com/maps/api/js?key={{ gmap_api_key }}&libraries=places&callback=initMap" async defer></script>
...
function addPerson(location, member_name, member_addr, membid, member_sex, member_alert) {
  ...
  if (membid == 0 && member_name == 'Me') {
    ...
    if (member_sex == 'M') {
      marker.setIcon('/static/healthcare/img/map/man-red.png')
    } else {
      marker.setIcon('/static/healthcare/img/map/woman-red.png')
    }
  } else {
    if (member_sex == 'M') {
      marker.setIcon('/static/healthcare/img/map/man-green.png')
    } else {
      marker.setIcon('/static/healthcare/img/map/woman-green.png')
    }
  }
  ...
  google.maps.event.addListener(marker, 'click', function() {
    ...
    //     .children('div')
    //     .show();
    $("#wid_map_search .jarviswidget-toggle-btn").click();
...
var nextAddress = 0;
// need memberAddresses because geocode is asynchronous
var memberAddresses = []
    ...
    $.getJSON("/provider_members/" + $person_id, {},
      function(json){
        ...
        $.each(json, function (index, value) {
            ...
            member_addr = value.address + ", " + value.city + ", " + value.state + " " + value.zip;
            member_id = value.id;
            member_alert = value.alert;
            member_sex = value.sex;
            // console.log(nm + addr);
            memberAddresses.push([member_name, member_addr, member_id, member_sex, member_alert]);
            // console.log(memberAddresses[i])
// alert("geocode member:" + member_addr);
//                   geocoder.geocode({'address': member_addr}, function(results, status) {
//                     if (status === 'OK') {
// // alert("member:" + memberAddresses[j][1]);
//                       // console.log("--"+nm+j+results)
//                       // console.log(j+"--"+memberAddresses[j])
//                       // resultsMap.setCenter(results[0].geometry.location);
//                       // addPerson(results[0].geometry.location, member_name, member_name + " " + member_addr, member_id)
//                       addPerson(results[0].geometry.location, memberAddresses[j][0], memberAddresses[j][1], memberAddresses[j][2], memberAddresses[j][3], memberAddresses[j][4])
//                       j++
//                     } else {
...
function getAddress(search, next) {
  globalGeocoder.geocode({address:search}, function (results,status)
    {
...
// console.log('nextAddress: ' + nextAddress + " memberAddresses.length: " + memberAddresses.length + " search: " + search + " member: " + memberAddresses[nextAddress-1][0])
        addPerson(p, memberAddresses[nextAddress-1][0], memberAddresses[nextAddress-1][1], memberAddresses[nextAddress-1][2], memberAddresses[nextAddress-1][3], memberAddresses[nextAddress-1][4])
      }
...
function theNext() {
  if (nextAddress < memberAddresses.length) {
    setTimeout('getAddress("'+memberAddresses[nextAddress][1]+'",theNext)', delay);
    nextAddress++;
  } else {
