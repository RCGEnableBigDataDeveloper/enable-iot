...
public class MetadataBuilder {
	...
	public static Level build(final DatalakeConnectionFactory datalakeConnectionFactory,
		...
		level.setText(entry.getKey().replace('\\', '/').toLowerCase());
		level.setName(entry.getKey().replace('\\', '/').toLowerCase());
		level.setType(type);
		...
		if (results != null) {
			for (Entry<ItemDefinition, List<ItemDefinition>> result : results.entrySet()) {
				...
				childLevel.setText(result.getKey().getName().replace('\\', '/').toLowerCase());
				childLevel.setName(result.getKey().getName().replace('\\', '/').toLowerCase());
				...
				if (result.getValue() != null) {
					for (ItemDefinition def : result.getValue()) {
						...
						leaf.setText(def.getName().replace('\\', '/').toLowerCase());
						leaf.setName(def.getName().replace('\\', '/').toLowerCase());
	...
	static void getChildren(final ItemDefinition def, final Level leaf) {
		...
		for (ItemDefinition childItem : childItems) {
			...
			leaf1.setText(childItem.getName().replace('\\', '/').toLowerCase());
			leaf1.setName(childItem.getName().replace('\\', '/').toLowerCase());
