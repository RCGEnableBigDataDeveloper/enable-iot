...
public class DatalakeConfiguration {
	...
	private ConcurrentMap<String, String> schemaReaders;
	...
	void configure() {
		...
		try {
			...
			this.schemaReaders = new ConcurrentHashMap<String, String>();
			...
			if (System.getProperty(DATALAKE_CONF) != null){
				...
				doc = db.parse(getClass().getResourceAsStream("/datalake-site.xml"));

			doc.getDocumentElement().normalize();

			NodeList properties = doc.getElementsByTagName("property");
			for (int i = 0; i < properties.getLength(); ++i) {
				String name = (((Element) properties.item(i)).getElementsByTagName("name").item(0).getTextContent().trim());
				String value = (((Element) properties.item(i)).getElementsByTagName("value").item(0).getTextContent().trim());
			...
			for (int i = 0; i < transformations.getLength(); ++i) {
				...
				String name = (((Element) transformations.item(i)).getElementsByTagName("name").item(0).getTextContent().trim());
				transformation.setName(name);
				String type = (((Element) transformations.item(i)).getElementsByTagName("type").item(0).getTextContent().trim());
				transformation.setType(type);
				String className = (((Element) transformations.item(i)).getElementsByTagName("class").item(0).getTextContent().trim());
			...
			for (int i = 0; i < schemaReaders.getLength(); ++i) {
				String name = (((Element) schemaReaders.item(i)).getElementsByTagName("name").item(0).getTextContent().trim());
				String value = (((Element) schemaReaders.item(i)).getElementsByTagName("type").item(0).getTextContent().trim());
				this.properties.put(name, value);
				this.schemaReaders.put(name != null ? name : "", value);
			}
		...
		logger.info("# of conections is : " + connections.getLength());
			...
			for (int i = 0; i < connections.getLength(); ++i) {
				...
				String name = (((Element) connections.item(i)).getElementsByTagName("name").item(0).getTextContent().trim());
				connection.setName(name != null ? name : "");

				String user = (((Element) connections.item(i)).getElementsByTagName("user").item(0).getTextContent().trim());
				connection.setUser(user != null ? user : "");
				String pwd = (((Element) connections.item(i)).getElementsByTagName("pwd").item(0).getTextContent().trim());
				try {
					...
					if (secretKey == null || "".equals(secretKey)) {
						...
						connection.setPwd(pwd != null ? EncryptDecryptUtil.decrypt(keyStream, pwd) : "");
					} else {
						connection.setPwd(pwd != null ? EncryptDecryptUtil.decrypt(secretKey, pwd) : "");
					}
				...
				String type = (((Element) connections.item(i)).getElementsByTagName("type").item(0).getTextContent().trim());
				connection.setType(type != null ? type : "");
				String port = (((Element) connections.item(i)).getElementsByTagName("port").item(0).getTextContent().trim());
				connection.setPort(port != null ? port : "");

				String host = (((Element) connections.item(i)).getElementsByTagName("host").item(0).getTextContent().trim());
				connection.setHost(host != null ? host : "");

				String path = (((Element) connections.item(i)).getElementsByTagName("path").item(0).getTextContent().trim());
				connection.setPath(path != null ? path : "");

				String clazz = (((Element) connections.item(i)).getElementsByTagName("class").item(0).getTextContent().trim());
				...
				if (extra.getLength() > 0) {
					...
					for (int j = 0; j < extra.getLength(); ++j) {
						String propName = (((Element) extra.item(j)).getElementsByTagName("name").item(0).getTextContent().trim());
						String propValue = (((Element) extra.item(j)).getElementsByTagName("value").item(0).getTextContent().trim());
			...
			for (int i = 0; i < services.getLength(); ++i) {
				ConnectionConfig connection = new ConnectionConfig();
				String name = (((Element) services.item(i)).getElementsByTagName("name").item(0).getTextContent().trim());
				connection.setName(name != null ? name : "");

				String user = (((Element) services.item(i)).getElementsByTagName("user").item(0).getTextContent().trim());
				connection.setUser(user != null ? user : "");
	...
	public ConcurrentMap<String, String> getSchemaReaders() {
		return schemaReaders;
	}

	public void setSchemaReaders(ConcurrentMap<String, String> schemaReaders) {
